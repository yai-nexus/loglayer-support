...
/**
 * 客户端专用日志功能 - 使用新的框架预设 API
 * 使用 createBrowserLogger 提供强大的浏览器端日志功能
 */

import { createBrowserLoggerSync } from '@yai-nexus/loglayer-support'
import type { IBrowserLogger, LogMetadata } from '@yai-nexus/loglayer-support'

// 创建浏览器端日志器实例
const createClientLogger = (): IBrowserLogger => {
...
export const logPageLoad = () => {
  ...
  clientLogger.logPerformance('page-load', loadTime, {
    url: window.location.href,
    userAgent: navigator.userAgent
  });
};

export const logUserAction = (action: string, element: string, metadata: LogMetadata = {}) => {
