...
export class SlsRestClient {
   ...
   * 生成阿里云签名
   */
  private generateSignature(method: string, uri: string, headers: Record<string, string>): string {
    // 【调试增强】记录签名生成的详细过程
    ...
    internalLogger.debug('StringToSign 构建详情', {
      ...
      stringToSign_escaped: normalizedStringToSign
        .replace(/\n/g, '\\n')
        .replace(/\r/g, '\\r')
        .replace(/\t/g, '\\t'),
      ...
      stringToSign_parts: normalizedStringToSign
        .split('\n')
   ...
   */
  private buildStringToSign(method: string, uri: string, headers: Record<string, string>): string {
  ...
  private getStandardizedDateString(): string {
    ...
    internalLogger.debug('时间标准化详情', {
      ...
      // 验证时间格式的正则表达式
      isValidHttpDate: /^[A-Za-z]{3}, \d{2} [A-Za-z]{3} \d{4} \d{2}:\d{2}:\d{2} GMT$/.test(
        utcString
      ),
    });
