...
test.describe('createBrowserLogger E2E Tests', () => {
  ...
  test('应该能够记录性能日志', async () => {
    await page.goto(
      'data:text/html,<html><head><title>Performance Test</title></head><body></body></html>'
    );
    ...
    const consoleMessages = (page as any).consoleMessages;
    expect(
      consoleMessages.some((msg) => msg.includes('[PERF]') && msg.includes('test-operation'))
    ).toBe(true);
  });

  test('应该能够处理错误对象', async () => {
    await page.goto(
      'data:text/html,<html><head><title>Error Test</title></head><body></body></html>'
    );
    ...
    const consoleMessages = (page as any).consoleMessages;
    expect(
      consoleMessages.some((msg) => msg.includes('捕获到测试错误') && msg.includes('测试错误'))
    ).toBe(true);
  });

  test('应该能够使用 localStorage 输出', async () => {
    await page.goto(
      'data:text/html,<html><head><title>LocalStorage Test</title></head><body></body></html>'
    );
  ...
  test('应该能够生成和管理会话 ID', async () => {
    await page.goto(
      'data:text/html,<html><head><title>Session Test</title></head><body></body></html>'
    );
  ...
  test('应该能够处理全局错误捕获', async () => {
    await page.goto(
      'data:text/html,<html><head><title>Global Error Test</title></head><body></body></html>'
    );
    ...
    const consoleMessages = (page as any).consoleMessages;
    expect(
      consoleMessages.some((msg) => msg.includes('全局错误捕获') && msg.includes('全局测试错误'))
    ).toBe(true);
  });
});
