...
# 2. 运行 Node.js 示例
echo "=== 2. 运行 Node.js 示例 (nodejs) ==="
if ./scripts/run-nodejs-example.sh; then
    echo "✅ Node.js 示例测试完成"
    NODEJS_SUCCESS=true
else
    echo "❌ Node.js 示例测试失败"
...
if [[ -f "logs/nodejs.log" ]]; then
    echo "✅ nodejs.log 已生成"
    echo "📄 文件大小: $(wc -l < logs/nodejs.log) 行"
    echo "🔍 最后几行内容:"
    tail -3 logs/nodejs.log | sed 's/^/   /'
    echo ""
else
    echo "❌ nodejs.log 未生成"
fi
...
for logfile in logs/basic.log logs/nodejs.log logs/nextjs.log; do
    if [[ -f "$logfile" ]]; then
...
if [[ "$NODEJS_SUCCESS" == "true" ]]; then
    echo "   ✅ Node.js 示例 (nodejs) - 成功"
else
    echo "   ❌ Node.js 示例 (nodejs) - 失败"
fi
...
if [[ "$NODEJS_SUCCESS" == "true" ]]; then SUCCESS_COUNT=$((SUCCESS_COUNT + 1)); fi
if [[ "$NEXTJS_SUCCESS" == "true" ]]; then SUCCESS_COUNT=$((SUCCESS_COUNT + 1)); fi

echo "📊 成功率: $SUCCESS_COUNT/3 个示例成功"
...
echo "   ./scripts/run-nodejs-example.sh  # Node.js 示例"
